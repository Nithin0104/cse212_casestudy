...
#include<malloc.h>
#include<string.h>
#define size 5 //total passengers 
# define MAX_SIZE 20

typedef struct NODE   //for each passenger detail
{
	int reg_no;
	int age;
	char name[MAX_SIZE];
...
int reserve()
{
	...
	else 
	{
		...
		temp=start;
		int i=1;
		if(temp->reg_no == 0) //if there is nothing in the list we add the first node
		{
			temp->reg_no = 1;
		...
		{
			if(temp->reg_no != i++)  //breaks if there is not a continuous numbering
				break;
		...
		if(num<size) //to add on the node to the available seats
		{
			num++;
			i++;
			if(temp->reg_no == (prev->reg_no+1) || i == 2) // If there is only one ticket re4served or to the end
			{
				new_node->reg_no = i;
				temp->next = new_node;
				return i;
...
int main()
{
	...
	{	
	    ...
	    case 2:   printf("\nGive the Registration number to be cancelled : "); //to cancel
	              scanf("%d", &reg);
	              if(reg>num)
